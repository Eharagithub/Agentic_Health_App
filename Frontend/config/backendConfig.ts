...
// 3. default to http://192.168.1.25:8001 for local testing
  ...
  try {
    ...
    if (dbg && typeof dbg === 'string') {
      ...
      if (host && host !== 'localhost' && host !== '127.0.0.1') {
        return `http://${host}:8001`;  // Orchestration Agent port
      }
    }
  } catch (e) {
    // ignore
...
const resolvedUrl = envBackendUrl() || debuggerHostBackend() || 'http://10.72.243.156:8001';
...
//   your machine LAN IP, e.g. "http://192.168.1.5:8001" so the device can reach it.
...
// - Orchestration Agent runs on port 8001 (Prompt Processor on 8000)